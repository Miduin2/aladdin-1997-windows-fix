...
bool IsAladdinInputProcedure(const WNDPROC procedure) noexcept
{
    ...
    // These are the seven keyboard and three joystick edit-control procedures
    // installed by the original ALADDINW.EXE input property pages. The
    // executable has no relocation for these absolute callbacks, but compare
    // as RVAs for clarity and safety.
    ...
    const HWND window, const int index, const LONG newValue)
{
    ...
    if (index == GWL_WNDPROC)
    {
        ...
        if (IsAladdinInputProcedure(gameProcedure))
        {
