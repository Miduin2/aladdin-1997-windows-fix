...
namespace
{
constexpr wchar_t kLauncherMutex[] = L"Local\\GameVault.DisneyClassicAladdin.Launcher";
...
std::wstring Utf16FromSystemMessage(const DWORD error)
{
    ...
    std::wstring message = length && raw ? std::wstring(raw, length) : L"Error desconocido";
    if (raw)
    {
        LocalFree(raw);
    }
...
bool IsAladdinRunning()
{
    ...
    if (Process32FirstW(snapshot, &entry))
    {
        do
        {
            if (_wcsicmp(entry.szExeFile, L"ALADDINW.EXE") == 0)
            {
                running = true;
                break;
            }
...
int WINAPI wWinMain(HINSTANCE, HINSTANCE, PWSTR, int)
{
    ...
    const std::wstring virtualGame = virtualRoot + L"ALADDINW.EXE";
